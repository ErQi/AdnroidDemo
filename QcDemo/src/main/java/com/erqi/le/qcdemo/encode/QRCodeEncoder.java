...
package com.erqi.le.qcdemo.encode;
...
import com.erqi.le.qcdemo.Contents;
import com.erqi.le.qcdemo.Intents;
import com.erqi.le.qcdemo.R;
...
public final class QRCodeEncoder {
        ...
        switch (type) {
            ...
            case Contents.Type.CONTACT:
                ...
                if (contactBundle != null) {
                    ...
                    String[] encoded = encoder.encode(Collections.singletonList(name), organization, Collections
                            .singletonList(address), phones, phoneTypes, emails, urls, note);
    ...
    private void encodeQRCodeContents(AddressBookParsedResult contact) {
        ContactEncoder encoder = useVCard ? new VCardContactEncoder() : new MECARDContactEncoder();
        String[] encoded = encoder.encode(toList(contact.getNames()), contact.getOrg(), toList(contact
                .getAddresses()), toList(contact.getPhoneNumbers()), null, toList(contact.getEmails()), toList(contact
                .getURLs()), null);
        // Make sure we've encoded at least one field.
        if (!encoded[1].isEmpty()) {
            contents = encoded[0];
